...
(tests
 (__ (quote [[A B C D] [A C D B] [B A D C] [D C A B]])) := {}
 (__ (quote [[A B C D E F] [B C D E F A] [C D E F A B] [D E F A B C] [E F A B C D] [F A B C D E]])) := {6 1}
 (__ (quote [[A B C D] [B A D C] [D C B A] [C D A B]])) := {4 1, 2 4}
 (__ (quote [[B D A C B] [D A B C A] [A B C A B] [B C A B C] [A D B C A]])) := {3 3}
